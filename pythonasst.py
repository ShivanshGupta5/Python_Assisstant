...
def showscore(Mode, mscore):  # display record
    with open("Snake_Game.csv", "r") as fo:
        ...
        print("Your Score =", mscore,
              "Highest score ever acheieved =", max(moderec))
...
def insert():  # Inserting feedback data into mysql table
    ...
    db = mysql.connector.connect(
        host="localhost", user="root", passwd="root@123", database="python_assistant")
    cur = db.cursor()
    ...
    cmd = cur.execute(
        f"insert into FEEDBACK VALUES('{name}' , {conno} , '{email}' , '{feedback}') ")
...
def output():  # Showing all the records of the feeback table
    ...
    try:
        db = mysql.connector.connect(
            host="localhost", user=user, password=passw, db="python_assistant")

    except:
        row3 = tk.Label(win1, text="Incorrect username or password...", font=(
            "Helvetica", 10), bg='white', fg='red')
        ...
        for data in records:
            for j in range(len(data)):
                put = data[j]
                e = tk.Entry(win2, width=25, text=data[j], font=(
                    "Helvetica", 22), bg='purple', fg='yellow')
...
while True:
    ...
    if ('play') in command:
        ...
        talk("Do you want to search using voice (Yes/No)? Kindly speak python before your answer ")
        res = take_command()
        if res == "No" or res == "no":
            talk("Shifting to keyboard mode")
            obj = input("Pleaase enter what do you want to shop today?")
            obj = obj.replace(" ", "+")
        else:
            talk("What would you like to shop today?")
            obj = take_command()
            obj = obj.replace(" ", "+")
        urll = "https://www.amazon.in/s?k="+obj
        ...
        conv = input(
            'Enter the language you want to convert the text into:').lower()
        ...
        row1 = tk.Label(win1, text="Username :", font=(
            "Helvetica", 10), bg='white', fg='black')
        row1.place(x=50, y=20)

        username = tk.Entry(win1, width=35)
        username.place(x=150, y=20, width=100)

        row2 = tk.Label(win1, text="Password :", font=(
            "Helvetica", 10), bg='white', fg='black')
